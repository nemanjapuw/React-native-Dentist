...
class LabDentalNewService1 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    componentDidMount() {
        ...
        if (!Utils.isEmpty(file)) {
            ...
            if (!Utils.isEmpty(file.file_list3))
                this.setState({ fileList3: file.file_list3 });
            if (!Utils.isEmpty(file.file_list4))
                this.setState({ fileList4: file.file_list4 });
        ...
        if (!Utils.isEmpty(store.getState().origin.origin)) {
            ...
            if (!Utils.isEmpty(origin)) {
                if (!Utils.isEmpty(origin.file)) {
                    if (!Utils.isEmpty(origin.file.file_list1))
                        this.setState({ fileList1: file.file_list1 });
                    if (!Utils.isEmpty(origin.file.file_list2))
                        this.setState({ fileList2: file.file_list2 });
                    if (!Utils.isEmpty(origin.file.file_list3))
                        this.setState({ fileList3: file.file_list3 });
                    if (!Utils.isEmpty(origin.file.file_list4))
                        this.setState({ fileList4: file.file_list4 });
                    this.setState({ file_text: file.file_text });
                }
            }
        ...
        if (this._isMounted) {
            try {
                ...
                RNFS.readFile(file_url, "base64").then(result => {
                    ...
                    if (cond == '3') file.id = this.state.fileList3.length + 1;
                    if (cond == '4') file.id = this.state.fileList4.length + 1;
                    file.name = file_name;
                    file.type = file_type;
                    file.size = file_size;
                    ...
                    if (cond == '3') {
                        let savedFileList = this.state.fileList3;
                        savedFileList.push(file);
                        this.setState({ fileList3: savedFileList });
                    }
                    if (cond == '4') {
                        let savedFileList = this.state.fileList4;
                        savedFileList.push(file);
                        this.setState({ fileList4: savedFileList });
                    }
    ...
    completeBtn() {
        ...
        file.file_list3 = this.state.fileList3;
        file.file_list4 = this.state.fileList4;
        this.props.navigation.navigate('LabDentalNewService', { file1: file });
    }
    ...
    render() {
        ...
        const renderItem = ({ item, index }) => (
            <View style={styles.item}>
                <Image source={Icons.icon_new} style={{ width: 40, height: 30 }} />
                    ...
                    <ImageBackground source={Images.signin_back} style={styles.signup_back}>
                        <View style={styles.topview}>
                            ...
                            <Text style={styles.main_text}>
                                {Utils.translate("Ldentalnewservcie1.label")}
                            </Text>
                        </View>
                                ...
                                <ScrollView>
                                    ...
                                    <View>
                                        <Text style={[styles.title, { textAlign: 'center' }]}>
                                            {Utils.translate("Ldentalnewservcie1.title1")}
                                        </Text>
                                        <Text style={styles.second_title}>
                                            {Utils.translate("Ldentalnewservcie1.desc1")}
                                        </Text>
                                    </View>
                                    ...
                                    <View style={{ paddingTop: 25 }}>
                                        <Text style={[styles.title, { textAlign: 'center' }]}>
                                            {Utils.translate("Ldentalnewservcie1.title2")}
                                        </Text>
                                        <Text style={styles.second_title}>
                                            {Utils.translate("Ldentalnewservcie1.desc2")}
                                        </Text>
                                    </View>
                                    <View style={{ backgroundColor: 'white', borderRadius: 15, height: 120, width: "100%", marginTop: 5 }}>
                                        <FlatList
                                    ...
                                    <View style={{ paddingTop: 25 }}>
                                        <Text style={[styles.title, { textAlign: 'center' }]}>
                                            {Utils.translate("Ldentalnewservcie1.title3")}
                                        </Text>
                                        <Text style={styles.second_title}>
                                            {Utils.translate("Ldentalnewservcie1.desc3")}
                                        </Text>
                                    </View>
                                    <View style={{ backgroundColor: 'white', borderRadius: 15, height: 120, width: "100%", marginTop: 5 }}>
                                        <FlatList
                                            horizontal={true}
                                            data={this.state.fileList3}
                                            ListEmptyComponent={renderNoItem}
                                            renderItem={renderItem}
                                            keyExtractor={item => item.id}
                                        />
                                        ...
                                        />
                                        <View style={{ width: '100%', paddingRight: 5, alignItems: 'center' }}>
                                            <TouchableOpacity onPress={this.SingleFilePicker.bind(this, '3')} >
                                                {/* <TouchableOpacity onPress={() => { this.props.navigation.navigate('DentalServiceFile'); }}> */}
                                                <Image
                                                    source={Icons.icon_save}
                                                    style={{ width: 40, height: 40, margin: 5, alignItems: 'flex-end' }}
                                    ...
                                    <View style={{ paddingTop: 25 }}>
                                        <Text style={[styles.title, { textAlign: 'center' }]}>
                                            {Utils.translate("Ldentalnewservcie1.title4")}
                                        </Text>
                                        <Text style={styles.second_title}>
                                            {Utils.translate("Ldentalnewservcie1.desc4")}
                                        </Text>
                                    </View>
                                    <View style={{ backgroundColor: 'white', borderRadius: 15, height: 120, width: "100%", marginTop: 5 }}>
                                        <FlatList
                                            horizontal={true}
                                            data={this.state.fileList4}
                                            ListEmptyComponent={renderNoItem}
                                            renderItem={renderItem}
                                            keyExtractor={item => item.id}
                                        />
                                        ...
                                        />
                                        <View style={{ width: '100%', paddingRight: 5, alignItems: 'center' }}>
                                            <TouchableOpacity onPress={this.SingleFilePicker.bind(this, '4')} >
                                                {/* <TouchableOpacity onPress={() => { this.props.navigation.navigate('DentalServiceFile'); }}> */}
                                                <Image
                                                    source={Icons.icon_save}
                                                    style={{ width: 40, height: 40, margin: 5, alignItems: 'flex-end' }}
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{ width: "100%" }}>
                                            ...
                                            >
                                                {Utils.translate("Ldentalnewservcie1.continue")}
                                            </Button>
                                        </View>
                                        <View style={{ flexDirection: "row", textAlign: "center", alignItems: "center", marginTop: 10 }}>
                                            ...
                                            <Text style={styles.signup_labelSecond}>
                                                {Utils.translate("Ldentalnewservcie1.page2")}
                                            </Text>
                                        </View>
                                    </View>
                                </ScrollView>
...
export default connect(null, mapDispatchToProps)(LabDentalNewService1);
